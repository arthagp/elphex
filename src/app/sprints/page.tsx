...
export default function SprintsPage() {
  const { activeSprint, tasks, user, pet, showXpGain, activeWorkspaceId } = useElphexStore();
  ...
  const sprintTasks = activeSprint
    ? tasks.filter((t) => t.projectId === activeSprint.projectId && t.status !== "DELETED")
    : [];
    ...
    <AppLayout>
      <div className="max-w-5xl mx-auto space-y-8 pb-12">
        ...
        {activeWorkspaceId?.includes("personal") ? (
          <div className="p-12 text-center glass-panel rounded-3xl border border-dashed border-slate-800 text-slate-400 space-y-4 max-w-2xl mx-auto mt-8 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-br from-red-500/5 to-violet-500/5 rounded-full blur-2xl pointer-events-none"></div>
            <div className="text-4xl animate-bounce">🐘🔒</div>
            <h3 className="font-extrabold text-sm uppercase tracking-wider text-slate-200 mt-2">Herd Mode Sprints Dikunci</h3>
            <p className="text-xs leading-relaxed text-slate-500 max-w-md mx-auto">
              Herd Mode membutuhkan kolaborasi kelompok. Silakan pindah ke **Ruang Kerja Organisasi (Team/Organization Workspace)** pada menu sidebar untuk berkolaborasi dan menyelesaikan Sprint bersama anggota tim Anda secara real-time!
            </p>
          </div>
        ) : (
          <>
